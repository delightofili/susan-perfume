function StatusBadge({ status }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-inter border ${styles[status] ?? "bg-white/5 text-white/40 border-white/10"}`}
    >
      <span
        className={`w-1.5 h-1.5 rounded-full ${dots[status] ?? "bg-white/30"}`}
      />
      {status}
    </span>
  );
