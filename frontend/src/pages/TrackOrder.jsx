...
const STEPS = ["Pending", "Pending Payment", "Paid", "Processing", "Shipped", "Delivered"];
...
function TrackOrder() {
    ...
    useEffect(() => {
        if (prefilled?.reference) doSearch(prefilled.reference);
    }, []);
    ...
    useEffect(() => {
        ...
        const channel = supabase
            ...
            .on(
                'postgres_changes',
                { event: 'UPDATE', schema: 'public', table: 'orders', filter: `reference=eq.${order.reference}` },
                (payload) => {
    ...
    const doSearch = async (refOverride) => {
        const ref = (refOverride || reference).trim().toUpperCase().replace(/\s+/g, '');
        ...
        <section className="min-h-screen w-full relative pb-20">
            ...
            <div className="relative z-40 max-w-lg mx-auto px-4 py-16">
                <div className="flex flex-col items-center mb-10">
                    <h1 className="text-3xl md:text-4xl font-playfair text-[#1a0a10] dark:text-[#f5e6a8] mb-4">Track Order</h1>
                    <GoldDividerDot />
                ...
                <div className="rounded-2xl border border-[#c9a84c]/20 bg-white/60 dark:bg-white/[0.03] backdrop-blur-sm p-6 mb-6 shadow-xl">
                    <div className="flex flex-col gap-4">
                        ...
                        >
                            {loading ? <span className="w-4 h-4 border-2 border-[#1a0a10]/30 border-t-[#1a0a10] rounded-full animate-spin" /> : "Verify Status"}
                        </button>
                    </div>
                </div>
                ...
                {order && (
                    <div className="rounded-2xl border border-[#c9a84c]/20 bg-white/60 dark:bg-white/[0.03] backdrop-blur-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="px-6 py-4 border-b border-[#c9a84c]/15 flex items-center justify-between bg-gold/5">
                            <div>
                                <p className="font-inter text-[10px] text-[#1a0a10]/40 dark:text-warm-cream/40 uppercase tracking-widest">Reference</p>
                                <p className="font-playfair text-lg text-[#1a0a10] dark:text-[#c9a84c] font-bold">{order.reference}</p>
                            </div>
                        ...
                        <div className="p-6 flex flex-col gap-8">
                            ...
                            <div className="grid grid-cols-2 gap-y-6 gap-x-4">
                                {[
                                    ...
                                    { label: "Amount Paid", value: "₦" + Number(order.total).toLocaleString(), gold: true },
                                    { label: "Items", value: `${order.items} Product(s)` },
                                    { label: "Order Date", value: new Date(order.created_at).toLocaleDateString("en-NG", { day: "numeric", month: "short", year: "numeric" }) },
                                ].map(({ label, value, gold }) => (
                                    <div key={label}>
                                        <p className="font-inter text-[10px] text-[#1a0a10]/40 dark:text-warm-cream/30 uppercase tracking-wider mb-1">{label}</p>
                                        <p className={`font-inter text-sm ${gold ? "font-bold text-[#c9a84c]" : "text-[#1a0a10] dark:text-[#f5e6a8]"}`}>{value}</p>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div>
                                <p className="font-inter text-[10px] text-[#1a0a10]/40 dark:text-warm-cream/40 uppercase tracking-widest mb-6">Delivery Timeline</p>
                                    ...
                                    {STEPS.map((step, i) => {
                                        ...
                                        return (
                                            <div key={step} className="flex md:flex-col items-center gap-4 md:gap-2 relative z-10">
                                                <div className={`w-8 h-8 rounded-full border flex items-center justify-center text-[10px] font-bold transition-all duration-1000 
                                                ...
                                                <div className="flex flex-col md:items-center">
                                                    <p className={`font-inter text-[11px] font-medium tracking-wide ${isDone ? "text-[#1a0a10] dark:text-[#c9a84c]" : "text-[#1a0a10]/30 dark:text-warm-cream/20"}`}>
                                                        {step}
                                                    </p>
                                                    {isCurrent && <span className="text-[9px] text-[#c9a84c]/60 animate-bounce md:hidden">Current Stage</span>}
                                                </div>
                                            </div>
                                        );
                                    })}
                ...
                {searched && !order && !loading && !error && (
                    <div className="text-center py-10 animate-pulse">
                        <p className="text-[#1a0a10]/40 dark:text-warm-cream/40 font-inter text-sm italic">No records found. Please check your reference.</p>
                    </div>
                )}

                <div className="mt-8 text-center">
                    <Link to="/shop" className="font-cinzel text-[10px] tracking-[2px] text-[#1a0a10]/40 dark:text-warm-cream/40 hover:text-[#c9a84c] transition-colors">
                        ← Back to Boutique
                    </Link>
                </div>
            </div>
